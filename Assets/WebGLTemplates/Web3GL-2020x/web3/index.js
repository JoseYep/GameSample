...
//Claim function
async function customSign(winner, loser, gameId, key) {
  const sha = web3.utils.soliditySha3(
    " ' " + winner + " ' ",
    " ' " + loser + " ' ",
    " ' " + gameId + " ' "
);
  const sig = await web3.eth.accounts.sign(sha, key);
  //const response = await diceRoll_contract.claim(winner, loser, gameId, sig.signature, {from: winner});
  return sign;
}
